...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run a public observation investigation campaign from a frozen fixture.")
    ...
    parser.add_argument(
        ...
        action="append",
        dest="signal_ids",
        help="Signal id to include. Repeat for multiple signals. Defaults to a representative NOAA set.",
    )
    parser.add_argument(
        "--max-frequency-hz",
        type=float,
    ...
    print(f"OpenPlazma public observation campaign run: {result['runId']}")
    print(f"Run path: {result['runPath']}")
    print(f"Report artifact path: {result['reportArtifactPath']}")
